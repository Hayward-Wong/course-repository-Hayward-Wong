...
mfdata_sp <- mfdata[, .(easting, northing, Presence)]
coordinates(mfdata_sp) <- ~ easting + northing
crs(mfdata_sp) <- crs(elev)
...
biomdata <- BIOMOD_FormatingData(resp.var = mfdata_sp,
                                 expl.var = covsfit,
                                 resp.name="Marsh Fritillary")
plot(biomdata)
...
# Second model fit ----
biomdata2 <- BIOMOD_FormatingData(resp.var = mfdata_sp,
...
modellist <- grep("GLM|GBM|GAM|MARS", get_built_models(biomoutput2), value=TRUE)
...
biomEM_f <- BIOMOD_EnsembleForecasting(EM.output = biomEM,
                                       ...
                                       selected.models = 
                                         "Marsh.Fritillary_EMwmeanByROC_mergedAlgo_Full_AllData")

plot(biomEM_f)

biomEM_fpred <- get_predictions(biomEM_f)

writeRaster(biomEM_fpred, "./Output/Rasters/MF_ensemble_test5.tif", overwrite=TRUE)
