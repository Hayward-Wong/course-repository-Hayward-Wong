...
basal.area(x = trees_mlunguya$diam)
...
trees_mlunguya$ba <- basal.area(dbh = trees_mlunguya$diam)
...
mlunguya_height_list <- split(trees_mlunguya$height, trees_mlunguya$family)
lapply(mlunguya_height_list, mean, na.rm = TRUE)
sapply(mlunguya_height_list, mean, na.rm = TRUE)
...
LPI <- read.csv("CC-5-fun-and-loop-master/LPI_data_loops.csv")
